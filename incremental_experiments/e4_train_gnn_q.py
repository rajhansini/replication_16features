"""E4 — bidirectional message passing (3-gene GNN-Q). Fixes R1 from the
harsh-reviewer pass: the original GNN only sends messages parent->child, so
an ancestor node (Grandfather: no incoming edges) never receives ANY
relational context — its embedding after N rounds is a purely local,
static transform of its own raw features, informed by nothing about its
descendants. Every training example (Trio/Nuclear) has this same blind
spot, so it's not something the network could have learned around.

Root/ancestor nodes are exactly who DP wants tested first most of the time
(11/12 3-gene configs want TEST Grandfather at root) — so the node type the
policy needs to reason about best is the one the architecture starves.

Fix: each message-passing round now aggregates BOTH parent->child AND
child->parent messages (separate learned message functions, since "what my
parent knows" and "what my child knows" are semantically different), summed
before the update function. Ancestor nodes now get descendant-aware
embeddings for the first time.

Everything else — CE+MSE loss (lambda_ce=1.0, matching E2), state-grouped
batching (matching E1/E2), data, seeds — is held fixed relative to E2, so
E2 vs E4 isolates this architectural change alone.

Usage:
    python e4_train_gnn_q.py --seed 0 --epochs 500
"""
...
gnn_mod = _load_module("gnn_run_e4", EXP_ROOT / "gnn" / "run.py")
...
RESULTS_BASE = HERE / "results" / "e4_gnn_bidir"
RESULTS_BASE.mkdir(parents=True, exist_ok=True)


class GNNQBidir(nn.Module):
    """Same head/hidden dims as gnn_q.GNNQ. Each round aggregates messages
    in BOTH directions along every parent->child edge (separate learned
    functions for forward and backward), so nodes with no incoming edges
    (ancestors) still receive relational context — from their descendants.
    """

    def __init__(self, node_feat=NODE_FEAT, hidden=HIDDEN, cost_dim=COST_DIM, n_rounds=N_ROUNDS):
        super().__init__()
        self.n_rounds = n_rounds
        self.msg_fwd = nn.ModuleList()   # parent -> child (down the tree)
        self.msg_bwd = nn.ModuleList()   # child -> parent (up the tree)
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0,
    ...
    log(f"\n{'='*60}\n[E4-GNN-Q-BIDIR] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "gnn_q_e4.pt")
        log(f"    saved -> {RESULTS_DIR/'gnn_q_e4.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "gnn_q_e4.pt", map_location=dev))
        ...
        (RESULTS_DIR / "results_e4.json").write_text(json.dumps(results, indent=2))
        log(f"    saved -> {RESULTS_DIR/'results_e4.json'}")
