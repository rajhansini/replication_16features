"""E5 four-way action log: DP (ground truth) vs MYOPIC vs GNN-Q vs MLP-Q.
GNN uses the E5 checkpoints (n_rounds=3 -- winner of the E5 scout sweep over
n_rounds in {1,2,3,4}, holding CE+MSE loss / state-grouped batching /
bidirectional message passing fixed at E4's settings). MLP reuses E2's
checkpoint unchanged (n_rounds only applies to the GNN's message passing).

Same walk-and-log structure as log_e4_fourway.py, pointed at the E5
(rounds=3) GNN checkpoints.

Outputs:
    results/{2gene|3gene}/seed{S}/e5_fourway.log
    results/{2gene|3gene}/seed{S}/e5_fourway.json

Usage:
    python log_e5_fourway.py --genes 3 --seed 0
    python log_e5_fourway.py --genes 2 --seed 0
"""
...
def run_3gene(seed, device, log):
    gnn_mod = _load_module("gnn_run_e5_3", EXP_ROOT / "gnn" / "run.py")
    mlp_mod = _load_module("mlp_run_e5_3", EXP_ROOT / "mlp" / "run.py")
    gnn_q   = _load_module("gnn_q_e5_rounds3_3", HERE / "e5_train_gnn_q.py")
    mlp_q   = _load_module("mlp_q_e5_3", Q_DIR / "mlp_q.py")
    ...
    if not gnn_wt.exists():
        raise FileNotFoundError(f"missing E5 weights: {gnn_wt} — run e5_train_gnn_q.py --n_rounds 3 first")
    if not mlp_wt.exists():
        raise FileNotFoundError(f"missing E2 weights: {mlp_wt}")

    gmodel = gnn_q.GNNQBidir(n_rounds=N_ROUNDS_E5).to(device)
...
def run_2gene(seed, device, log):
    tg      = _load_module("two_gene_run_e5_2", EXP_ROOT / "two_gene" / "run.py")
    gnn_q   = _load_module("two_gene_gnn_q_e5_rounds3_2", HERE / "e5_train_two_gene_gnn_q.py")
    mlp_q   = _load_module("two_gene_mlp_q_e5_2", Q_DIR / "two_gene_mlp_q.py")
    ...
    if not gnn_wt.exists():
        raise FileNotFoundError(f"missing E5 weights: {gnn_wt} — run e5_train_two_gene_gnn_q.py --n_rounds 3 first")
    if not mlp_wt.exists():
        raise FileNotFoundError(f"missing E2 weights: {mlp_wt}")
...
def main():
    ...
    log_path = out_dir / "e5_fourway.log"
    ...
    log(f"[e5_fourway] {datetime.now().isoformat()}  genes={args.genes}  seed={args.seed}"
        f"  variant=E5 (GNN: n_rounds=3, winner of scout sweep; MLP: unchanged from E2)")
    ...
    payload = {
        "genes": args.genes, "seed": args.seed, "variant": "E5_gnn_rounds3",
        "overall_whole_traj": {
    ...
    (out_dir / "e5_fourway.json").write_text(json.dumps(payload, indent=2))
    log(f"saved -> {out_dir/'e5_fourway.json'}")
