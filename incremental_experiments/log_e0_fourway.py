"""E0 (baseline) four-way action log: DP (ground truth) vs MYOPIC vs GNN-Q vs
MLP-Q, original unmodified MSE-trained checkpoints — the rung the ablation
roadmap (see fixing_gnn_q artifact §07) starts from.

Every other rung in incremental_experiments/ builds on this one. Existing
comparisons only ever covered 3 of these 4 policies for the baseline models
(GNN/MLP vs MYOPIC, via things_to_improve_Q_star_experiments/action_compare)
— DP was never in the same table. This script closes that gap: one walk,
one state, four picks, logged together.

Walks MYOPIC's most-likely-outcome trajectory (deterministic, model-free) and
logs all four policies' picks at every visited state — not just the first
action, the whole trajectory.

Outputs:
    results/{2gene|3gene}/seed{S}/e0_fourway.log
    results/{2gene|3gene}/seed{S}/e0_fourway.json

Usage:
    python log_e0_fourway.py --genes 3 --seed 0
    python log_e0_fourway.py --genes 2 --seed 0
"""
...
def walk_and_log(state0, q_hat_gnn, q_hat_mlp, policy_dp, individuals, belief,
                  genes, config, v_stop_cache, log, max_steps=20):
...
def run_3gene(seed, device, log):
    gnn_mod = _load_module("gnn_run_e0_3", EXP_ROOT / "gnn" / "run.py")
    mlp_mod = _load_module("mlp_run_e0_3", EXP_ROOT / "mlp" / "run.py")
    gnn_q   = _load_module("gnn_q_e0_3", Q_DIR / "gnn_q.py")
    mlp_q   = _load_module("mlp_q_e0_3", Q_DIR / "mlp_q.py")
    from qsa_data import build_qsa_index  # noqa: F401

    gnn_wt = Q_DIR / "results" / "seed_runs" / f"seed{seed}" / "gnn_q.pt"
    mlp_wt = Q_DIR / "results" / "seed_runs" / f"seed{seed}" / "mlp_q.pt"
    if not gnn_wt.exists():
        raise FileNotFoundError(f"missing baseline weights: {gnn_wt}")
    if not mlp_wt.exists():
        raise FileNotFoundError(f"missing baseline weights: {mlp_wt}")

    gmodel = gnn_q.GNNQ().to(device)
    ...
    for key in gnn_mod.TEST_KEYS:
        ...
        summary[key] = walk_and_log(frozenset(), q_hat_gnn, q_hat_mlp, policy_dp,
                                     individuals, belief, genes, config, v_stop_cache, log)
...
def run_2gene(seed, device, log):
    tg      = _load_module("two_gene_run_e0_2", EXP_ROOT / "two_gene" / "run.py")
    gnn_q   = _load_module("two_gene_gnn_q_e0_2", Q_DIR / "two_gene_gnn_q.py")
    mlp_q   = _load_module("two_gene_mlp_q_e0_2", Q_DIR / "two_gene_mlp_q.py")

    gnn_wt = Q_DIR / "results_2gene" / "seed_runs" / f"seed{seed}" / "gnn_q_2gene.pt"
    mlp_wt = Q_DIR / "results_2gene" / "seed_runs" / f"seed{seed}" / "mlp_q_2gene.pt"
    if not gnn_wt.exists():
        raise FileNotFoundError(f"missing baseline weights: {gnn_wt}")
    if not mlp_wt.exists():
        raise FileNotFoundError(f"missing baseline weights: {mlp_wt}")
...
def main():
    ...
    log_path = out_dir / "e0_fourway.log"
    ...
    log(f"[e0_fourway] {datetime.now().isoformat()}  genes={args.genes}  seed={args.seed}  variant=E0 (baseline MSE)")
    ...
    payload = {
        "genes": args.genes, "seed": args.seed, "variant": "E0_baseline_MSE",
        "overall_whole_traj": {
    ...
    (out_dir / "e0_fourway.json").write_text(json.dumps(payload, indent=2))
    log(f"saved -> {out_dir/'e0_fourway.json'}")
