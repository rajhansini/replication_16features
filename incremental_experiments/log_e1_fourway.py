"""E1 four-way action log: DP (ground truth) vs MYOPIC vs GNN-Q vs MLP-Q,
E1 checkpoints (state-grouped batching, loss still plain MSE — the batching
effect isolated from the loss-function effect, see e1_train_*.py).

Same walk-and-log structure as log_e0_fourway.py, pointed at the E1
checkpoints instead of the original baseline ones.

Outputs:
    results/{2gene|3gene}/seed{S}/e1_fourway.log
    results/{2gene|3gene}/seed{S}/e1_fourway.json

Usage:
    python log_e1_fourway.py --genes 3 --seed 0
    python log_e1_fourway.py --genes 2 --seed 0
"""
...
def run_3gene(seed, device, log):
    gnn_mod = _load_module("gnn_run_e1_3", EXP_ROOT / "gnn" / "run.py")
    mlp_mod = _load_module("mlp_run_e1_3", EXP_ROOT / "mlp" / "run.py")
    gnn_q   = _load_module("gnn_q_e1_3", Q_DIR / "gnn_q.py")
    mlp_q   = _load_module("mlp_q_e1_3", Q_DIR / "mlp_q.py")
    from qsa_data import build_qsa_index  # noqa: F401

    gnn_wt = HERE / "results" / "e1_gnn_mse_grouped" / "seed_runs" / f"seed{seed}" / "gnn_q_e1.pt"
    mlp_wt = HERE / "results" / "e1_mlp_mse_grouped" / "seed_runs" / f"seed{seed}" / "mlp_q_e1.pt"
    if not gnn_wt.exists():
        raise FileNotFoundError(f"missing E1 weights: {gnn_wt} — run e1_train_gnn_q.py first")
    if not mlp_wt.exists():
        raise FileNotFoundError(f"missing E1 weights: {mlp_wt} — run e1_train_mlp_q.py first")
...
def run_2gene(seed, device, log):
    tg      = _load_module("two_gene_run_e1_2", EXP_ROOT / "two_gene" / "run.py")
    gnn_q   = _load_module("two_gene_gnn_q_e1_2", Q_DIR / "two_gene_gnn_q.py")
    mlp_q   = _load_module("two_gene_mlp_q_e1_2", Q_DIR / "two_gene_mlp_q.py")

    gnn_wt = HERE / "results" / "e1_gnn_mse_grouped_2gene" / "seed_runs" / f"seed{seed}" / "gnn_q_e1_2gene.pt"
    mlp_wt = HERE / "results" / "e1_mlp_mse_grouped_2gene" / "seed_runs" / f"seed{seed}" / "mlp_q_e1_2gene.pt"
    if not gnn_wt.exists():
        raise FileNotFoundError(f"missing E1 weights: {gnn_wt} — run e1_train_two_gene_gnn_q.py first")
    if not mlp_wt.exists():
        raise FileNotFoundError(f"missing E1 weights: {mlp_wt} — run e1_train_two_gene_mlp_q.py first")
...
def main():
    ...
    log_path = out_dir / "e1_fourway.log"
    ...
    log(f"[e1_fourway] {datetime.now().isoformat()}  genes={args.genes}  seed={args.seed}"
        f"  variant=E1 (MSE + state-grouped batching)")
    ...
    payload = {
        "genes": args.genes, "seed": args.seed, "variant": "E1_mse_grouped_batching",
        "overall_whole_traj": {
    ...
    (out_dir / "e1_fourway.json").write_text(json.dumps(payload, indent=2))
    log(f"saved -> {out_dir/'e1_fourway.json'}")
