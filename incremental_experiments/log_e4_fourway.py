"""E2 four-way action log: DP (ground truth) vs MYOPIC vs GNN-Q vs MLP-Q,
E2 checkpoints (state-grouped batching + cross-entropy, lambda_ce=1.0 —
same batching as E1, loss now includes the ranking term. These are the
same checkpoints trained by fixing_gnn_q/train_*_ce.py; no new training
needed for this rung.

Same walk-and-log structure as log_e0_fourway.py / log_e4_fourway.py,
pointed at the E2 (fixing_gnn_q ce-variant) checkpoints.

Outputs:
    results/{2gene|3gene}/seed{S}/e4_fourway.log
    results/{2gene|3gene}/seed{S}/e4_fourway.json

Usage:
    python log_e4_fourway.py --genes 3 --seed 0
    python log_e4_fourway.py --genes 2 --seed 0
"""
...
def run_3gene(seed, device, log):
    gnn_mod = _load_module("gnn_run_e4_3", EXP_ROOT / "gnn" / "run.py")
    mlp_mod = _load_module("mlp_run_e4_3", EXP_ROOT / "mlp" / "run.py")
    gnn_q   = _load_module("gnn_q_e4_bidir_3", HERE / "e4_train_gnn_q.py")
    mlp_q   = _load_module("mlp_q_e4_3", Q_DIR / "mlp_q.py")
    ...
    if not gnn_wt.exists():
        raise FileNotFoundError(f"missing E4 weights: {gnn_wt} — run e4_train_gnn_q.py first")
    if not mlp_wt.exists():
        raise FileNotFoundError(f"missing E2 weights: {mlp_wt}")

    gmodel = gnn_q.GNNQBidir().to(device)
...
def run_2gene(seed, device, log):
    tg      = _load_module("two_gene_run_e4_2", EXP_ROOT / "two_gene" / "run.py")
    gnn_q   = _load_module("two_gene_gnn_q_e4_bidir_2", HERE / "e4_train_two_gene_gnn_q.py")
    mlp_q   = _load_module("two_gene_mlp_q_e4_2", Q_DIR / "two_gene_mlp_q.py")
    ...
    if not gnn_wt.exists():
        raise FileNotFoundError(f"missing E4 weights: {gnn_wt} — run e4_train_two_gene_gnn_q.py first")
    if not mlp_wt.exists():
        raise FileNotFoundError(f"missing E2 weights: {mlp_wt}")
...
def main():
    ...
    log_path = out_dir / "e4_fourway.log"
    ...
    log(f"[e4_fourway] {datetime.now().isoformat()}  genes={args.genes}  seed={args.seed}"
        f"  variant=E4 (GNN: bidirectional message passing; MLP: unchanged from E2)")
    ...
    payload = {
        "genes": args.genes, "seed": args.seed, "variant": "E4_gnn_bidirectional_mp",
        "overall_whole_traj": {
    ...
    (out_dir / "e4_fourway.json").write_text(json.dumps(payload, indent=2))
    log(f"saved -> {out_dir/'e4_fourway.json'}")
