"""E1 — state-grouped batching, loss still plain MSE (2-gene GNN-Q).
See e1_train_gnn_q.py (3-gene) for the full rationale.

Usage:
    python e1_train_two_gene_gnn_q.py --seed 0 --epochs 500
"""
...
tg      = _load_module("two_gene_run_e1g", EXP_ROOT / "two_gene" / "run.py")
two_gnn = _load_module("two_gene_gnn_q_e1_base", Q_DIR / "two_gene_gnn_q.py")
from qsa_data import build_qsa_index  # noqa: E402
from exputils.eval import q_rollout  # noqa: E402

RESULTS_BASE = HERE / "results" / "e1_gnn_mse_grouped_2gene"
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=0.0, mode="both", seed=0):
    ...
    log(f"\n{'='*60}\n[E1-2GENE-GNN-Q-MSE-GROUPED] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "gnn_q_e1_2gene.pt")
        log(f"    saved -> {RESULTS_DIR/'gnn_q_e1_2gene.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "gnn_q_e1_2gene.pt", map_location=dev))
