"""E9 four-way action log: DP (ground truth) vs MYOPIC (Kanix's canonical
myopic_greedy) vs GNN-Q vs MLP-Q. GNN uses the E9 combined-pooling
(global-sum + local-neighbor-mean concat) checkpoints; MLP-Q reuses E2's
checkpoint unchanged (E9 is GNN-only, same as E6/E7).

Same walk-and-log structure as log_e7_fourway.py.

Outputs:
    results/{2gene|3gene}/seed{S}/e9_fourway.log
    results/{2gene|3gene}/seed{S}/e9_fourway.json

Usage:
    python log_e9_fourway.py --genes 3 --seed 0
    python log_e9_fourway.py --genes 2 --seed 0
"""
...
def model_pick_gnn(state, q_hat, individuals, belief, genes, config):
...
def walk_and_log(state0, q_hat_gnn, q_hat_mlp, policy_dp, individuals, belief,
    ...
    for step in range(max_steps):
        ...
        gk, gp = model_pick_gnn(state, q_hat_gnn, individuals, belief, genes, config)
        mk, mp = model_pick_gnn(state, q_hat_mlp, individuals, belief, genes, config)
...
def run_3gene(seed, device, log):
    gnn_mod = _load_module("gnn_run_e9_3", EXP_ROOT / "gnn" / "run.py")
    mlp_mod = _load_module("mlp_run_e9_3", EXP_ROOT / "mlp" / "run.py")
    gnn_q   = _load_module("gnn_q_e9_combo_3", HERE / "e9_train_gnn_q.py")
    mlp_q   = _load_module("mlp_q_e9_3", Q_DIR / "mlp_q.py")
    from qsa_data import build_qsa_index  # noqa: F401

    gnn_wt = HERE / "results" / "e9_gnn_combopool" / "seed_runs" / f"seed{seed}" / "gnn_q_e9.pt"
    mlp_wt = ROOT / "fixing_gnn_q" / "results" / "mlp_q_ce" / "seed_runs" / f"seed{seed}" / "mlp_q_ce.pt"
    if not gnn_wt.exists():
        raise FileNotFoundError(f"missing E9 weights: {gnn_wt}")
    if not mlp_wt.exists():
        raise FileNotFoundError(f"missing E2 weights: {mlp_wt}")

    gmodel = gnn_q.GNNQBidirComboPool().to(device)
    gmodel.load_state_dict(torch.load(gnn_wt, map_location=device))
    gmodel.eval()
    log(f"loaded {gnn_wt}")

    mmodel = mlp_q.MLPQ().to(device)
    ...
    for fam in gnn_mod.TRAIN_FAMILIES + gnn_mod.TEST_FAMILIES:
        ...
        nmask_np = gnn_q.build_neighbor_mask(edge_cache[fam], len(sample_ds["individuals"]))
        nmask_cache[fam] = torch.tensor(nmask_np, device=device)
    ...
    for key in gnn_mod.TEST_KEYS:
        ...
        q_hat_gnn = gnn_q.precompute_qhat(gmodel, ds, key, ei, nmask, device)
...
def run_2gene(seed, device, log):
    tg      = _load_module("two_gene_run_e9_2", EXP_ROOT / "two_gene" / "run.py")
    gnn_q   = _load_module("two_gene_gnn_q_e9_combo_2", HERE / "e9_train_two_gene_gnn_q.py")
    mlp_q   = _load_module("two_gene_mlp_q_e9_2", Q_DIR / "two_gene_mlp_q.py")

    gnn_wt = HERE / "results" / "e9_gnn_combopool_2gene" / "seed_runs" / f"seed{seed}" / "gnn_q_e9_2gene.pt"
    ...
    for fam in tg.TRAIN_FAMILIES + tg.TEST_FAMILIES:
        ...
        nmask_np = gnn_q.build_neighbor_mask(edge_cache[fam], len(individuals))
        nmask_cache[fam] = torch.tensor(nmask_np, device=device)
    ...
    for fam, reg, pre in tg.TEST_CONFIGS:
        ...
        ei = torch.tensor(edge_cache[fam], device=device)
        nmask = nmask_cache[fam]
        q_hat_gnn = gnn_q.precompute_qhat(gmodel, ds_g, key, ei, nmask, device)
...
def main():
    ...
    log_path = out_dir / "e9_fourway.log"
    ...
    log(f"[e9_fourway] {datetime.now().isoformat()}  genes={args.genes}  seed={args.seed}"
        f"  variant=E9 (GNN: combined global-sum + local-mean pooling; MLP: unchanged from E2)")
    ...
    payload = {
        "genes": args.genes, "seed": args.seed, "variant": "E9_combopool",
        "overall_whole_traj": {
    ...
    (out_dir / "e9_fourway.json").write_text(json.dumps(payload, indent=2))
    log(f"saved -> {out_dir/'e9_fourway.json'}")
