"""E4 — bidirectional message passing (2-gene GNN-Q). See e4_train_gnn_q.py
(3-gene) for the full rationale (fixes R1: ancestor nodes get zero
relational context under one-directional parent->child message passing).

Usage:
    python e4_train_two_gene_gnn_q.py --seed 0 --epochs 500
"""
...
tg = _load_module("two_gene_run_e4", EXP_ROOT / "two_gene" / "run.py")
...
RESULTS_BASE = HERE / "results" / "e4_gnn_bidir_2gene"
RESULTS_BASE.mkdir(parents=True, exist_ok=True)


class GNNQBidir(nn.Module):
    """See e4_train_gnn_q.py's GNNQBidir — same idea, 2-gene dims."""
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0, mode="both", seed=0):
    ...
    log(f"\n{'='*60}\n[E4-2GENE-GNN-Q-BIDIR] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "gnn_q_e4_2gene.pt")
        log(f"    saved -> {RESULTS_DIR/'gnn_q_e4_2gene.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "gnn_q_e4_2gene.pt", map_location=dev))
