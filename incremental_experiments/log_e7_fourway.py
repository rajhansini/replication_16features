"""E7 four-way action log: DP (ground truth) vs MYOPIC (Kanix's canonical
myopic_greedy) vs GNN-Q vs MLP-Q. GNN uses the E7 neighbor-mean-pooling
checkpoints; MLP-Q reuses E2's checkpoint unchanged (E7 is GNN-only so far
-- MLP-Q neighbor pooling is a follow-up, needs edge_index added to its
pipeline).

Same walk-and-log structure as log_e4_fourway.py / log_e6_fourway.py.

Outputs:
    results/{2gene|3gene}/seed{S}/e7_fourway.log
    results/{2gene|3gene}/seed{S}/e7_fourway.json

Usage:
    python log_e7_fourway.py --genes 3 --seed 0
    python log_e7_fourway.py --genes 2 --seed 0
"""
...
def run_3gene(seed, device, log):
    gnn_mod = _load_module("gnn_run_e7_3", EXP_ROOT / "gnn" / "run.py")
    mlp_mod = _load_module("mlp_run_e7_3", EXP_ROOT / "mlp" / "run.py")
    gnn_q   = _load_module("gnn_q_e7_neighbor_3", HERE / "e7_train_gnn_q.py")
    mlp_q   = _load_module("mlp_q_e7_3", Q_DIR / "mlp_q.py")
    ...
    if not gnn_wt.exists():
        raise FileNotFoundError(f"missing E7 weights: {gnn_wt}")
    if not mlp_wt.exists():
        raise FileNotFoundError(f"missing E2 weights: {mlp_wt}")

    gmodel = gnn_q.GNNQBidirNeighborPool().to(device)
...
def run_2gene(seed, device, log):
    tg      = _load_module("two_gene_run_e7_2", EXP_ROOT / "two_gene" / "run.py")
    gnn_q   = _load_module("two_gene_gnn_q_e7_neighbor_2", HERE / "e7_train_two_gene_gnn_q.py")
    mlp_q   = _load_module("two_gene_mlp_q_e7_2", Q_DIR / "two_gene_mlp_q.py")
...
def main():
    ...
    log_path = out_dir / "e7_fourway.log"
    ...
    log(f"[e7_fourway] {datetime.now().isoformat()}  genes={args.genes}  seed={args.seed}"
        f"  variant=E7 (GNN: neighbor-mean pooling; MLP: unchanged from E2)")
    ...
    payload = {
        "genes": args.genes, "seed": args.seed, "variant": "E7_neighborpool",
        "overall_whole_traj": {
    ...
    (out_dir / "e7_fourway.json").write_text(json.dumps(payload, indent=2))
    log(f"saved -> {out_dir/'e7_fourway.json'}")
