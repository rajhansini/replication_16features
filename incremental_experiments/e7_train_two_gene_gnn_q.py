"""E7 -- neighbor-mean pooling instead of global-mean pooling (2-gene GNN-Q).
See e7_train_gnn_q.py (3-gene) for the full rationale. Same GNNQBidirNeighborPool
architecture as the 3-gene version -- only NODE_FEAT/COST_DIM/HIDDEN dims differ.

Usage:
    python e7_train_two_gene_gnn_q.py --seed 0 --epochs 500
"""
...
tg = _load_module("two_gene_run_e7", EXP_ROOT / "two_gene" / "run.py")
...
RESULTS_BASE = HERE / "results" / "e7_gnn_neighborpool_2gene"
...
class GNNQBidirNeighborPool(nn.Module):
    ...
    candidate's direct 1-hop relatives, not the whole family."""
    ...
    def forward(self, node_feats, edge_index, cost_vec, action_idx, neighbor_mask):
        ...
        B = h.shape[0]
        pooled = pooled_all[torch.arange(B, device=h.device), action_idx]
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0, mode="both", seed=0):
    ...
    log(f"\n{'='*60}\n[E7-2GENE-GNN-Q-NEIGHBORPOOL] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "gnn_q_e7_2gene.pt")
        log(f"    saved -> {RESULTS_DIR/'gnn_q_e7_2gene.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "gnn_q_e7_2gene.pt", map_location=dev))
