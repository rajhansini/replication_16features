"""E7 -- neighbor-mean pooling instead of global-mean pooling (3-gene GNN-Q).
Everything else held fixed at E4's settings (bidirectional message passing,
CE+MSE loss, state-grouped batching) -- the ONLY change is what gets pooled
for the readout: E4 averages over ALL people in the family; E7 averages
over ONLY the candidate's direct 1-hop relatives (parents + children).
Aggregator stays MEAN in both -- this isolates pooling SCOPE as the single
new variable against E4, not scope+aggregator confounded together (that
confound is exactly what a sum-over-neighbors design would have created,
since E6 already changed the aggregator alone on the global scope).

Motivation: E6 (sum, global scope) helped MLP-Q in-distribution but hurt
generalization to the Extended family, likely because sum's magnitude
scales with family size while mean doesn't. A candidate's local neighbor
COUNT barely changes even when the family around them grows (most people
have 1-3 direct relatives regardless of total family size), so neighbor
pooling should be inherently more size-robust than global pooling of any
kind, while (unlike global mean) it's actually specific to who's being
scored instead of the same vector for every candidate in a state.

Usage:
    python e7_train_gnn_q.py --seed 0 --epochs 500
"""
...
gnn_mod = _load_module("gnn_run_e7", EXP_ROOT / "gnn" / "run.py")
...
RESULTS_BASE = HERE / "results" / "e7_gnn_neighborpool"
RESULTS_BASE.mkdir(parents=True, exist_ok=True)


def build_neighbor_mask(edge_index_np, n_people):
    """(N, N) float mask: mask[i, j] = 1 if j is i's direct parent or child,
    0 otherwise (including j == i -- self excluded, matches candidate being
    concatenated separately). edge_index is (2, E) parent(src) -> child(dst)."""
    mask = np.zeros((n_people, n_people), dtype=np.float32)
    src, dst = edge_index_np[0], edge_index_np[1]
    for s, d in zip(src, dst):
        mask[d, s] = 1.0  # child's neighbor includes parent
        mask[s, d] = 1.0  # parent's neighbor includes child
    return mask


class GNNQBidirNeighborPool(nn.Module):
    """Identical to E4's GNNQBidir except the readout pool averages only the
    candidate's direct 1-hop relatives, not the whole family. Requires the
    (N,N) neighbor_mask (built once per family, passed into forward)."""
    ...
    def forward(self, node_feats, edge_index, cost_vec, action_idx, neighbor_mask):
        h = self.embed(node_feats, edge_index)               # (B, N, hidden)
        # neighbor_mask: (N, N), row i = which people count as i's neighbors
        degree = neighbor_mask.sum(dim=1, keepdim=True).clamp(min=1.0)   # (N, 1)
        neighbor_mask_norm = neighbor_mask / degree                       # row-normalized -> mean
        pooled_all = torch.einsum('ij,bjd->bid', neighbor_mask_norm, h)   # (B, N, hidden) -- pooled_i for every possible i
        B = h.shape[0]
        pooled = pooled_all[torch.arange(B, device=h.device), action_idx]  # (B, hidden) -- pooled_{action_idx[b]} per row
        candidate = h[torch.arange(B, device=h.device), action_idx]        # (B, hidden)
...
def evaluate_rollout(model, keys, struct_cache, edge_cache, nmask_cache, device, log):
    results = {}
    for key in keys:
        fam = key.split("_")[0]
        with open(CACHE_DIR / f"{key}.pkl", "rb") as f:
        ...
        q_hat = precompute_qhat(model, ds, key, ei, nmask, device)
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0,
    ...
    log(f"\n{'='*60}\n[E7-GNN-Q-NEIGHBORPOOL] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    for fam in gnn_mod.TRAIN_FAMILIES + gnn_mod.TEST_FAMILIES:
        ...
        nmask_np = build_neighbor_mask(edge_cache[fam], len(sample_ds["individuals"]))
        nmask_cache[fam] = torch.tensor(nmask_np, device=dev)

    model = GNNQBidirNeighborPool().to(dev)
    ...
    if mode in ("train", "both"):
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "gnn_q_e7.pt")
        log(f"    saved -> {RESULTS_DIR/'gnn_q_e7.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "gnn_q_e7.pt", map_location=dev))
        log(f"\n[3] Eval on TEST_KEYS ({len(test_keys)} configs), "
            f"real greedy-Q(s,a) rollout vs exact DP (q_rollout)...")
        results, avg = evaluate_rollout(model, test_keys, struct_cache, edge_cache, nmask_cache, dev, log)
        (RESULTS_DIR / "results_e7.json").write_text(json.dumps(results, indent=2))
        log(f"    saved -> {RESULTS_DIR/'results_e7.json'}")
