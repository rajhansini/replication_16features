"""E1 — state-grouped batching, loss still plain MSE (3-gene MLP-Q).
See e1_train_gnn_q.py for the full rationale; identical except no
message-passing / edge_index.

Usage:
    python e1_train_mlp_q.py --seed 0 --epochs 500
"""
...
mlp_mod = _load_module("mlp_run_e1", EXP_ROOT / "mlp" / "run.py")
mlp_q   = _load_module("mlp_q_e1_base", Q_DIR / "mlp_q.py")
...
RESULTS_BASE = HERE / "results" / "e1_mlp_mse_grouped"
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=0.0,
    ...
    log(f"\n{'='*60}\n[E1-MLP-Q-MSE-GROUPED] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "mlp_q_e1.pt")
        log(f"    saved -> {RESULTS_DIR/'mlp_q_e1.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "mlp_q_e1.pt", map_location=dev))
