"""E6 — sum pooling instead of mean pooling (3-gene GNN-Q). Everything else
held fixed at E4's settings (bidirectional message passing, CE+MSE loss,
state-grouped batching) -- the ONLY change is `pooled = h.sum(dim=1)`
instead of `pooled = h.mean(dim=1)` in the readout step, so E4 vs E6
isolates the pooling operator alone.

Motivation: the original V-head model (gnn/run.py's class GNN, before the
Q-head variant existed) used sum pooling -- "Pool: sum over nodes -> (32,)".
The Q-head variant switched to mean with no documented reason. Sum also
matches the problem's actual structure better: the true value function
(r_reward) is a SUM over untested people's contributions, not an average --
mean pooling implicitly shrinks the family-wide summary as family size
grows, which is arguably the wrong invariance here.

Usage:
    python e6_train_gnn_q.py --seed 0 --epochs 500
"""
...
gnn_mod = _load_module("gnn_run_e6", EXP_ROOT / "gnn" / "run.py")
...
RESULTS_BASE = HERE / "results" / "e6_gnn_sumpool"
RESULTS_BASE.mkdir(parents=True, exist_ok=True)


class GNNQBidirSumPool(nn.Module):
    """Identical to E4's GNNQBidir except the readout pool is sum, not mean."""
    ...
    def forward(self, node_feats, edge_index, cost_vec, action_idx):
        ...
        pooled = h.sum(dim=1)  # <-- the only change from E4's GNNQBidir
        B = h.shape[0]
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0,
    ...
    log(f"\n{'='*60}\n[E6-GNN-Q-SUMPOOL] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "gnn_q_e6.pt")
        log(f"    saved -> {RESULTS_DIR/'gnn_q_e6.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "gnn_q_e6.pt", map_location=dev))
