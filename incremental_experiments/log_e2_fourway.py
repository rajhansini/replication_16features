...
Same walk-and-log structure as log_e0_fourway.py / log_e2_fourway.py,
pointed at the E2 (fixing_gnn_q ce-variant) checkpoints.

Outputs:
    results/{2gene|3gene}/seed{S}/e2_fourway.log
    results/{2gene|3gene}/seed{S}/e2_fourway.json

Usage:
    python log_e2_fourway.py --genes 3 --seed 0
    python log_e2_fourway.py --genes 2 --seed 0
"""
...
def run_3gene(seed, device, log):
    gnn_mod = _load_module("gnn_run_e2_3", EXP_ROOT / "gnn" / "run.py")
    mlp_mod = _load_module("mlp_run_e2_3", EXP_ROOT / "mlp" / "run.py")
    gnn_q   = _load_module("gnn_q_e2_3", Q_DIR / "gnn_q.py")
    mlp_q   = _load_module("mlp_q_e2_3", Q_DIR / "mlp_q.py")
    from qsa_data import build_qsa_index  # noqa: F401

    gnn_wt = ROOT / "fixing_gnn_q" / "results" / "gnn_q_ce" / "seed_runs" / f"seed{seed}" / "gnn_q_ce.pt"
    mlp_wt = ROOT / "fixing_gnn_q" / "results" / "mlp_q_ce" / "seed_runs" / f"seed{seed}" / "mlp_q_ce.pt"
    if not gnn_wt.exists():
        raise FileNotFoundError(f"missing E2 weights: {gnn_wt}")
...
def run_2gene(seed, device, log):
    tg      = _load_module("two_gene_run_e2_2", EXP_ROOT / "two_gene" / "run.py")
    gnn_q   = _load_module("two_gene_gnn_q_e2_2", Q_DIR / "two_gene_gnn_q.py")
    mlp_q   = _load_module("two_gene_mlp_q_e2_2", Q_DIR / "two_gene_mlp_q.py")

    gnn_wt = ROOT / "fixing_gnn_q" / "results" / "gnn_q_ce_2gene" / "seed_runs" / f"seed{seed}" / "gnn_q_ce_2gene.pt"
...
def main():
    ...
    log_path = out_dir / "e2_fourway.log"
    ...
    log(f"[e2_fourway] {datetime.now().isoformat()}  genes={args.genes}  seed={args.seed}"
        f"  variant=E2 (MSE + CE, state-grouped batching)")
    ...
    payload = {
        "genes": args.genes, "seed": args.seed, "variant": "E2_mse_ce_grouped_batching",
        "overall_whole_traj": {
    ...
    (out_dir / "e2_fourway.json").write_text(json.dumps(payload, indent=2))
    log(f"saved -> {out_dir/'e2_fourway.json'}")
