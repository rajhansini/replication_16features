"""E6 — sum pooling instead of mean pooling (2-gene GNN-Q). See
e6_train_gnn_q.py (3-gene) for the full rationale. Same GNNQBidir
architecture as E4 -- only the readout pool (sum vs mean) varies.

Usage:
    python e6_train_two_gene_gnn_q.py --seed 0 --epochs 500
"""
...
tg = _load_module("two_gene_run_e6", EXP_ROOT / "two_gene" / "run.py")
...
RESULTS_BASE = HERE / "results" / "e6_gnn_sumpool_2gene"
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0, mode="both", seed=0):
    ...
    log(f"\n{'='*60}\n[E6-2GENE-GNN-Q-SUMPOOL] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "gnn_q_e6_2gene.pt")
        log(f"    saved -> {RESULTS_DIR/'gnn_q_e6_2gene.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "gnn_q_e6_2gene.pt", map_location=dev))
