"""E6 — sum pooling instead of mean pooling (3-gene MLP-Q). Everything else
held fixed at E2's settings (CE+MSE loss, state-grouped batching). See
e6_train_gnn_q.py for the full rationale — same idea, applied to MLP-Q's
own readout (MLP-Q has no message passing, but it pools raw node features
the same way GNN-Q pools post-message-passing embeddings).

Usage:
    python e6_train_mlp_q.py --seed 0 --epochs 500
"""
...
mlp_mod = _load_module("mlp_run_e6", EXP_ROOT / "mlp" / "run.py")
mlp_q_base = _load_module("mlp_q_e6_base", Q_DIR / "mlp_q.py")
...
NODE_FEAT = mlp_q_base.NODE_FEAT
COST_DIM  = mlp_q_base.COST_DIM
CACHE_DIR = mlp_mod.CACHE_DIR

RESULTS_BASE = HERE / "results" / "e6_mlp_sumpool"
RESULTS_BASE.mkdir(parents=True, exist_ok=True)


class MLPQSumPool(nn.Module):
    """Identical to mlp_q.MLPQ except the readout pool is sum, not mean."""
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0,
    ...
    log(f"\n{'='*60}\n[E6-MLP-Q-SUMPOOL] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "mlp_q_e6.pt")
        log(f"    saved -> {RESULTS_DIR/'mlp_q_e6.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "mlp_q_e6.pt", map_location=dev))
