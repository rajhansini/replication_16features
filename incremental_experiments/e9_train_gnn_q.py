...
neighbor-mean readout (3-gene GNN-Q). Everything else held fixed at E4's
settings (bidirectional message passing, CE+MSE loss, state-grouped
batching) -- the ONLY change vs E7 is the head now also sees the global-sum
term, so the readout is [pooled_global, pooled_local, candidate, cost_vec]
instead of [pooled_local, candidate, cost_vec]. embed() (message passing) is
byte-identical to E4/E6/E7.

Motivation: neither E6 (sum, global) nor E7 (mean, local) dominated on the
Extended (OOD) comparison -- E6 won ratio2 in 3-gene but had the worst root-
match, E7 won step-agreement but was worst on root-match in 3-gene, E4 (plain
global mean) still won root-match outright in both gene counts. No single
pooling scope/aggregator is best everywhere. E9 tests whether giving the head
BOTH the whole-family sum AND the candidate-specific local mean lets it
recover E4's root-match robustness while keeping whatever value-closeness
gain sum pooling showed in-distribution, instead of trading one off for the
other.

Usage:
    python e9_train_gnn_q.py --seed 0 --epochs 500
"""
...
gnn_mod = _load_module("gnn_run_e9", EXP_ROOT / "gnn" / "run.py")
...
RESULTS_BASE = HERE / "results" / "e9_gnn_combopool"
...
class GNNQBidirComboPool(nn.Module):
    ...
    vector, instead of only the local one. Requires the same (N,N)
    neighbor_mask as E7 (built once per family, passed into forward)."""
    ...
    def forward(self, node_feats, edge_index, cost_vec, action_idx, neighbor_mask):
        ...
        pooled_global = h.sum(dim=1)                          # (B, hidden) -- E6's term, same for every action

        degree = neighbor_mask.sum(dim=1, keepdim=True).clamp(min=1.0)   # (N, 1)
        neighbor_mask_norm = neighbor_mask / degree                       # row-normalized -> mean
        pooled_all = torch.einsum('ij,bjd->bid', neighbor_mask_norm, h)   # (B, N, hidden)
        pooled_local = pooled_all[torch.arange(B, device=h.device), action_idx]  # (B, hidden) -- E7's term
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0,
    ...
    log(f"\n{'='*60}\n[E9-GNN-Q-COMBOPOOL] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "gnn_q_e9.pt")
        log(f"    saved -> {RESULTS_DIR/'gnn_q_e9.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "gnn_q_e9.pt", map_location=dev))
