"""GNN-Q retrained with combined MSE + cross-entropy loss (3-gene).

Same model architecture and same (state, action, Q*) data as
experiments_after_understanding/q_learning/gnn_q.py — only the training loss
changes. See losses.py for why: MSE alone never penalizes a flipped argmax
when the true Q-gap between candidates is small (root state is the worst
offender — 1 state out of >1M, with near-tied candidates).

Batches are now built at the STATE-group level (not flat rows), padded to
that config's max candidate count, so cross-entropy can be computed as one
vectorized softmax-over-candidates per state, in the same forward pass MSE
uses.

Usage:
    python train_gnn_q_ce.py --seed 0 --epochs 500 --lambda_ce 1.0
"""
...
gnn_mod = _load_module("gnn_run_ce", EXP_ROOT / "gnn" / "run.py")
gnn_q   = _load_module("gnn_q_ce_base", Q_DIR / "gnn_q.py")
...
RESULTS_BASE = HERE / "results" / "gnn_q_ce"
...
def train_one_epoch(model, ei, data, opt, groups_per_batch, lambda_ce, device):
    ...
    for start in range(0, n_groups, groups_per_batch):
        ...
        rows = group_rows[g_idx]              # (B, k_max)
        mask = group_mask[g_idx]               # (B, k_max)
        target = group_target[g_idx]           # (B,)

        # Only forward real (non-padded) rows through the model — padding to
        # k_max would otherwise waste compute on slots that get masked out
        # anyway (this was a 4x slowdown before the fix).
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0,
    ...
    log(f"\n{'='*60}\n[GNN-Q-CE] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        ckpt_path = RESULTS_DIR / "checkpoint_gnn_ce.pt"
        start_epoch = 1
        if ckpt_path.exists():
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "gnn_q_ce.pt")
        log(f"    saved -> {RESULTS_DIR/'gnn_q_ce.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "gnn_q_ce.pt", map_location=dev))
        ...
        (RESULTS_DIR / "results_gnn_ce.json").write_text(json.dumps(results, indent=2))
        log(f"    saved -> {RESULTS_DIR/'results_gnn_ce.json'}")
