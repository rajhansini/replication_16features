"""2-gene MLP-Q retrained with combined MSE + cross-entropy loss, PLUS
depth-based reweighting. See train_gnn_q_ce_reweighted.py (3-gene GNN
version) for the full rationale — this mirrors it exactly for 2-gene MLP-Q.

Usage:
    python train_two_gene_mlp_q_ce_reweighted.py --seed 0 --epochs 500 --lambda_ce 1.0
"""
...
tg      = _load_module("two_gene_run_mcew", EXP_ROOT / "two_gene" / "run.py")
two_mlp = _load_module("two_gene_mlp_q_cew_base", Q_DIR / "two_gene_mlp_q.py")
from qsa_data import build_qsa_index  # noqa: E402
from exputils.eval import q_rollout  # noqa: E402

RESULTS_BASE = HERE / "results" / "mlp_q_ce_reweighted_2gene"
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0, mode="both", seed=0):
    ...
    log(f"\n{'='*60}\n[2GENE-MLP-Q-CE-REWEIGHTED] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "mlp_q_ce_rw_2gene.pt")
        log(f"    saved -> {RESULTS_DIR/'mlp_q_ce_rw_2gene.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "mlp_q_ce_rw_2gene.pt", map_location=dev))
