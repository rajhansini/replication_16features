"""GNN-Q retrained with combined MSE + cross-entropy loss, PLUS depth-based
reweighting (3-gene).

This is train_gnn_q_ce.py (Fix 1: cross-entropy instead of plain MSE, so the
loss actually penalizes a flipped argmax) plus Fix 2: upweight shallow states
in that loss.

Why Fix 2 on top of Fix 1: cross-entropy already means every batched state
contributes one ranking term regardless of its candidate count. But root is
still just 1 state out of >1M, competing equally with every other single
state for a place in a batch — no different in expectation from any other
state. depth_weight() (see losses.py) makes root's own loss term count for
more once it IS in a batch: weight = 1/(1+depth), so root (depth 0) gets
weight 1.0, depth-1 states get 0.5, etc. Combined with Fix 1 this both (a)
scores root by ranking, not raw value, and (b) makes getting root wrong
actually cost the optimizer something.

Usage:
    python train_gnn_q_ce_reweighted.py --seed 0 --epochs 500 --lambda_ce 1.0
"""
...
gnn_mod = _load_module("gnn_run_cew", EXP_ROOT / "gnn" / "run.py")
gnn_q   = _load_module("gnn_q_cew_base", Q_DIR / "gnn_q.py")
...
RESULTS_BASE = HERE / "results" / "gnn_q_ce_reweighted"
...
def load_config_groups(key, struct_feats, edge_index, device, log):
    ...
    # slot 0 is always filled (every group has >=1 candidate) -> safe to read depth from it
    ...
    log(f"    [{key}] {len(ds['states']):,} states -> {len(y):,} (s,a) rows "
        ...
        f"root weight={depth_weight(torch.tensor(0.0)).item():.2f})")
    return {
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0,
    ...
    log(f"\n{'='*60}\n[GNN-Q-CE-REWEIGHTED] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        ckpt_path = RESULTS_DIR / "checkpoint_gnn_ce_rw.pt"
        start_epoch = 1
        if ckpt_path.exists():
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "gnn_q_ce_rw.pt")
        log(f"    saved -> {RESULTS_DIR/'gnn_q_ce_rw.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "gnn_q_ce_rw.pt", map_location=dev))
        ...
        (RESULTS_DIR / "results_gnn_ce_rw.json").write_text(json.dumps(results, indent=2))
        log(f"    saved -> {RESULTS_DIR/'results_gnn_ce_rw.json'}")
