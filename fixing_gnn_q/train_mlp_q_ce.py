"""MLP-Q retrained with combined MSE + cross-entropy loss (3-gene).

Same as train_gnn_q_ce.py but for MLP-Q — same bug applies (plain MSE never
penalizes a flipped argmax at root), same fix (state-grouped batches +
cross-entropy, see losses.py). MLP-Q has no message passing / edge_index, so
the forward signature is (node_feats, cost_vec, action_idx) — one arg
simpler than GNN-Q everywhere.

Usage:
    python train_mlp_q_ce.py --seed 0 --epochs 500 --lambda_ce 1.0
"""
...
mlp_mod = _load_module("mlp_run_ce", EXP_ROOT / "mlp" / "run.py")
mlp_q   = _load_module("mlp_q_ce_base", Q_DIR / "mlp_q.py")
...
RESULTS_BASE = HERE / "results" / "mlp_q_ce"
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0,
    ...
    log(f"\n{'='*60}\n[MLP-Q-CE] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        ckpt_path = RESULTS_DIR / "checkpoint_mlp_ce.pt"
        start_epoch = 1
        if ckpt_path.exists():
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "mlp_q_ce.pt")
        log(f"    saved -> {RESULTS_DIR/'mlp_q_ce.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "mlp_q_ce.pt", map_location=dev))
        ...
        (RESULTS_DIR / "results_mlp_ce.json").write_text(json.dumps(results, indent=2))
        log(f"    saved -> {RESULTS_DIR/'results_mlp_ce.json'}")
