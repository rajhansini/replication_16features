"""2-gene GNN-Q retrained with combined MSE + cross-entropy loss.

Same as train_gnn_q_ce.py (3-gene) — same model, same data, only the loss
and batching change (state-grouped, see losses.py). Mirrors
two_gene_gnn_q.py's data generation exactly.

Usage:
    python train_two_gene_gnn_q_ce.py --seed 0 --epochs 500 --lambda_ce 1.0
"""
...
tg      = _load_module("two_gene_run_ce", EXP_ROOT / "two_gene" / "run.py")
two_gnn = _load_module("two_gene_gnn_q_ce_base", Q_DIR / "two_gene_gnn_q.py")
from qsa_data import build_qsa_index  # noqa: E402
from exputils.eval import q_rollout  # noqa: E402

RESULTS_BASE = HERE / "results" / "gnn_q_ce_2gene"
...
def evaluate_rollout(model, configs, struct_cache, edge_cache, device, log):
    ...
    for fam, reg, pre in configs:
        ...
        ds = two_gnn.build_config(fam, reg, pre)
        ...
        q_hat = two_gnn.precompute_qhat(model, ds, key, edge_index_t, device)
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0, mode="both", seed=0):
    ...
    log(f"\n{'='*60}\n[2GENE-GNN-Q-CE] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "gnn_q_ce_2gene.pt")
        log(f"    saved -> {RESULTS_DIR/'gnn_q_ce_2gene.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "gnn_q_ce_2gene.pt", map_location=dev))
