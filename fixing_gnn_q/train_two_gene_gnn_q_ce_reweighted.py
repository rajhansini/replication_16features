"""2-gene GNN-Q retrained with combined MSE + cross-entropy loss, PLUS
depth-based reweighting. See train_gnn_q_ce_reweighted.py (3-gene) for the
full rationale — this mirrors it exactly, only the data generation differs
(on-the-fly via two_gene/run.py, same as train_two_gene_gnn_q_ce.py).

Usage:
    python train_two_gene_gnn_q_ce_reweighted.py --seed 0 --epochs 500 --lambda_ce 1.0
"""
...
tg      = _load_module("two_gene_run_cew", EXP_ROOT / "two_gene" / "run.py")
two_gnn = _load_module("two_gene_gnn_q_cew_base", Q_DIR / "two_gene_gnn_q.py")
from qsa_data import build_qsa_index  # noqa: E402
from exputils.eval import q_rollout  # noqa: E402

RESULTS_BASE = HERE / "results" / "gnn_q_ce_reweighted_2gene"
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0, mode="both", seed=0):
    ...
    log(f"\n{'='*60}\n[2GENE-GNN-Q-CE-REWEIGHTED] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        for fam, reg, pre in tg.TRAIN_CONFIGS:
            ...
            row_depths = state_depths(ds["states"], s_idx)
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "gnn_q_ce_rw_2gene.pt")
        log(f"    saved -> {RESULTS_DIR/'gnn_q_ce_rw_2gene.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "gnn_q_ce_rw_2gene.pt", map_location=dev))
