"""2-gene MLP-Q retrained with combined MSE + cross-entropy loss.

Same as train_mlp_q_ce.py (3-gene) — same model, same data, only the loss
and batching change (state-grouped, see losses.py). Mirrors
two_gene_mlp_q.py's data generation exactly.

Usage:
    python train_two_gene_mlp_q_ce.py --seed 0 --epochs 500 --lambda_ce 1.0
"""
...
tg      = _load_module("two_gene_run_mce", EXP_ROOT / "two_gene" / "run.py")
two_mlp = _load_module("two_gene_mlp_q_ce_base", Q_DIR / "two_gene_mlp_q.py")
from qsa_data import build_qsa_index  # noqa: E402
from exputils.eval import q_rollout  # noqa: E402

RESULTS_BASE = HERE / "results" / "mlp_q_ce_2gene"
...
def main(device="cpu", epochs=500, groups_per_batch=512, lambda_ce=1.0, mode="both", seed=0):
    ...
    log(f"\n{'='*60}\n[2GENE-MLP-Q-CE] {datetime.now().isoformat()}  mode={mode}  epochs={epochs}"
    ...
    if mode in ("train", "both"):
        ...
        torch.save(model.state_dict(), RESULTS_DIR / "mlp_q_ce_2gene.pt")
        log(f"    saved -> {RESULTS_DIR/'mlp_q_ce_2gene.pt'}")

    if mode in ("eval", "both"):
        if mode == "eval":
            model.load_state_dict(torch.load(RESULTS_DIR / "mlp_q_ce_2gene.pt", map_location=dev))
