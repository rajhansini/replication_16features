"""MLP baseline — V*(s) prediction without message passing.

Flow per state:
    node features (N_people x 13)
        -> sum pool over people -> (13,)
        -> concat cost vector   -> (24,)
        -> Linear(24->16) -> ReLU -> Linear(16->1)
        -> scalar V_hat(s)

No edges, no graph, no message passing.
Same training data and labels as step10 (step9 cache, V*(s) from exact DP).
Same train/test split: Trio + Nuclear -> ThreeGeneration.
"""
...
NODE_FEAT  = 3 * len(GENES) + 1 + 3   # 13: 9 beliefs + is_tested + n_parents + n_children + depth
...
class MLP(nn.Module):
    """Mean-pool node features, concat cost vector, predict V*(s)."""
    ...
    def forward(self, node_feats: torch.Tensor, cost_vec: torch.Tensor) -> torch.Tensor:
        ...
        """
        pooled = node_feats.mean(dim=1)           # (B, 13) — mean pool: size-invariant
        x      = torch.cat([pooled, cost_vec], dim=-1)  # (B, 24)
...
def run_eval(model, struct_cache, device, log):
