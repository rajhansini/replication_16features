"""Multi-seed variant of gnn/run.py — 3-gene GNN.

Separate script, does not modify or import gnn/run.py, so the original
script and its results/ outputs are untouched (backward compatible).
Only addition versus gnn/run.py: an explicit --seed argument
...
loss, eval/rollout) is identical to gnn/run.py.
"""
...
# ── structural features / edges (identical to gnn/run.py) ─────────────────────
...
# ── data loading (identical to gnn/run.py) ──────────────────────────────────────
...
# ── training (identical to gnn/run.py) ──────────────────────────────────────────
...
def run_eval(model, struct_cache, edge_cache, device, log, results_dir):
    import json
    model.eval()
    results = {}
...
def main(device="cpu", epochs=500, seed=0):
    ...
    log(f"\n{'='*60}")
    log(f"[GNN seed={seed}] {datetime.now().isoformat()}")
    ...
    log(f"    saved -> {results_dir}/gnn.pt")

    log(f"\n[3] Eval on ThreeGeneration (unseen)...")
    run_eval(model, struct_cache, edge_cache, device, log, results_dir)
