"""Step 6: GNN trained on Base + Aggressive presets.

Same architecture as step5 (node_feat_dim=7, hidden_dim=32, n_rounds=2).
6 TRAIN families (ThreeGeneration + Extended × LowHigh × {Base,Aggressive}
                  + ThreeGeneration × MediumEven × {Base,Aggressive})
2 TEST  families (Extended × MediumEven × {Base,Aggressive})

Run:
    python step6_aggressive/run.py
"""
...
# ─── state → graph (identical to step5) ─────────────────────────────────────
...
# ─── training (identical to step5) ──────────────────────────────────────────

def train_gnn(datasets_list, model, epochs=500, start_epoch=1,
              lr=1e-3, val_frac=0.2, device="cpu", print_every=50,
              ckpt_path=None, ckpt_every=50, history=None):
...
def compute_ratio2_gnn(model, ds, device="cpu"):
    ...
    def _get_entry(state):
        ...
        marg = entry
        return marg, lift_tuple_posteriors_to_genes(entry, genes, GENOTYPE_STATES), None
    ...
    def value_at(state):
        ...
        best_net_q, best_person = v_stop, None
        for i in individuals:
            ...
            if r_i + exp > best_net_q:
                best_net_q, best_person = r_i + exp, i
        ...
        exp_true = sum(p * value_at(frozenset(state | {(best_person, g)}))
                       for g, p in pmf_best.items()
                       if p > 1e-12 and frozenset(state | {(best_person, g)}) in belief)
...
# 6 TRAIN: ThreeGen × {Base,Aggressive} × {LowHigh,MediumEven} + Extended × {Base,Aggressive} × LowHigh
# 2 TEST:  Extended × {Base,Aggressive} × MediumEven
TRAIN_KEYS = {cfg_key(c) for c in CONFIGS if not (c["family_label"] == "Extended" and c["allele_freqs"]["GeneA"] == 0.08)}


def main(device="cpu", target_epochs=300, fresh=False):
    ...
    # Reuse step5 cache for the 4 Base configs to avoid redundant computation
    ...
    for cfg in CONFIGS:
        ...
        # Reuse existing step5 cache for Base configs
    ...
    # ── 2. Train GNN ─────────────────────────────────────────────────────────
    log(f"\n[2] Training GNN (node_feat_dim=7, hidden_dim=32, n_rounds=2):")
    ...
    if start_epoch <= target_epochs:
        ...
        for cfg in CONFIGS:
            ...
            for state, v_star in ds["V_star"].items():
                ...
                samples.append((nf, ei, float(v_star)))
            del ds
        log(f"    Train samples: {len(samples)} (from {len(TRAIN_KEYS)} families)")
    ...
    log(f"\n[3] Evaluation — done {len(all_results)}/8, pending {len(pending)}/8")
    ...
    summary = {
        "model": "PedigreeGNN", "node_feat_dim": 7, "hidden_dim": 32, "n_rounds": 2,
        "train_families": sorted(TRAIN_KEYS),
    ...
    log(f"\n  {'Key':<55} {'Split':<6} {'ratio2':>8}")
    log(f"  {'-'*72}")
    for key, r in all_results.items():
        log(f"  {key:<55} {r['split']:<6} {r['ratio2']:>8.6f}")
