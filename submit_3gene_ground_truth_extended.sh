#!/bin/bash
#SBATCH --job-name=3gene_gt_extended
#SBATCH --output=fresh_dataset/3gene/slurm_extended_%A_task%a.log
#SBATCH --time=01:00:00
#SBATCH --mem=256G
#SBATCH --cpus-per-task=4
#SBATCH --partition=general
#SBATCH --array=0-11
#SBATCH --requeue

# 3-gene ground truth: Extended family, one config per task (no seed loop --
# DP/myopic don't depend on any trained checkpoint, so 12 tasks not 36).
# Each config builds its belief map from scratch (~27min/~100GB, proven twice
# already this session for E4/E6 evaluation).
cd /net/projects/ranalab/rajhansini/replication_16features

REGIMES=(LowHigh MediumEven LowLow HighHigh MixedA MixedB)
PRESETS=(Base Aggressive)

REGIME=${REGIMES[$(( SLURM_ARRAY_TASK_ID / 2 ))]}
PRESET=${PRESETS[$(( SLURM_ARRAY_TASK_ID % 2 ))]}

echo "task=$SLURM_ARRAY_TASK_ID -> regime=$REGIME preset=$PRESET"

/net/projects/ranalab/rajhansini/conda_envs/genetic-rl/bin/python \
  incremental_experiments/build_3gene_ground_truth.py --family Extended --regime "$REGIME" --preset "$PRESET"
